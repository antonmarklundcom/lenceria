...
export function HomeCategoryShowcase({ categories }: { categories: readonly { id: number; slug: string; name: string }[] }) {
  return <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:py-20"><div>
    <h2 data-reveal="scroll" className="font-serif text-[30px] font-medium">{t("home.categorias.titulo")}</h2>
    ...
    <div className="mt-8 grid grid-cols-2 gap-4 lg:grid-cols-4 lg:gap-6">
      {categories.map((category, index) => <Link data-reveal="card" style={{ "--i": index } as CSSProperties} key={category.id} href={`/categoria/${category.slug}`} className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-secondary">
