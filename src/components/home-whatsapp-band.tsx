...
export function HomeWhatsappBand() {
  ...
  return <section className="relative isolate overflow-hidden py-24 sm:py-32">
    ...
    <div className="mx-auto max-w-6xl px-4 text-center text-white sm:px-6"><HomeFade><h2 className="font-serif text-[36px] leading-tight font-medium">{t("home.whatsapp.titulo")}</h2><p className="mx-auto mt-4 max-w-md text-[15px] leading-7">{t("home.whatsapp.texto")}</p>{whatsapp ? <a href={whatsapp} className="mt-8 inline-flex min-h-12 items-center rounded-full bg-white px-8 py-4 text-sm text-foreground">{t("home.whatsapp.boton")}</a> : null}</HomeFade></div>
  </section>;
}